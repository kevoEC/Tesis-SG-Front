...
export default function ProspectoForm() {
  ...
  const handleSubmit = (e) => {
    ...
    navigate("/prospectos/vista");
  };

  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        <Button variant="link" onClick={() => navigate("/prospectos/vista")} className="text-blue-600 px-0">
        ...
        <CardContent className="p-8">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <FormGroup label="Tipo de Identificación">
              <Select onValueChange={(val) => handleChange("idTipoIdentificacion", val)}>
            ...
            <FormGroup label="Origen del Cliente">
              <Select onValueChange={(val) => handleChange("idOrigenCliente", val)}>
                <SelectTrigger><SelectValue placeholder="Seleccionar..." /></SelectTrigger>
                <SelectContent className="bg-white">
            ...
            <FormGroup label="Producto de Interés">
              <Select onValueChange={(val) => handleChange("idProductoInteres", val)}>
                <SelectTrigger><SelectValue placeholder="Seleccionar..." /></SelectTrigger>
                <SelectContent className="bg-white">
            ...
            <FormGroup label="Agencia">
              <Select onValueChange={(val) => handleChange("idAgencia", val)}>
                <SelectTrigger><SelectValue placeholder="Seleccionar..." /></SelectTrigger>
                <SelectContent className="bg-white">
                  {catalogos.agencias.map((item) => (
                    <SelectItem key={item.id} value={item.id.toString()}>{item.ciudad}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </FormGroup>
